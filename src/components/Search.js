...
import { Container, Dropdown, FormControl, Button, Image, Row, Col, FormGroup } from 'react-bootstrap';
import '../index.css';
            ...
            <Row className="align-items-center">
              <Col xs={12} sm={8} md={9} lg={10}>
                <Dropdown variant="" size="lg">
                  <FormGroup>
                    <Dropdown.Toggle id="dropdown-basic" size="lg" variant="" style={{ border: 0 }} active>
                      <FormControl
                    ...
                    <Dropdown.Menu className="container-fluid" id="myUL" aria-labelledby="dropdownMenuButton">
                      {filteredItems.map((item) => (
                        <Dropdown.Item key={item.id} className="dropdown-item item-search-li" onClick={() => setInputElement(item.name)}>
                          <Image src={item.imgSrc} height="50" width="50" className="drow" />
                          {item.name}
                        </Dropdown.Item>
                      ))}
                    </Dropdown.Menu>
              ...
              <Col xs={12} sm={4} md={3} lg={2} className="text-center mt-3 mt-sm-0">
